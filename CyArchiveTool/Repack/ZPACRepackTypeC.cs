...
{
    internal class ZPACRepackTypeC
    {
        public static void RepackMultiple(string packFile, string unpackedDir, string virtualDirectory, string pathSeparatorChar)
        {
            ...
            if (hasRepacked)
            {
                ...
                Console.WriteLine("");
                Console.WriteLine($"Finished repacking specificed directory to '{Path.GetFileName(packFile)}' file");
            }
            else
            {
                SharedFunctions.IfFileExistsDel(packDataFile);
